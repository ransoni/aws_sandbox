...
def create_lex_intents(intents, account_id, slot_type_version):
    """
    Creates Lex intents.
...
def create_lex_bot(lex_bot, account_id):
    """
    Creates Lex Bot.
    ...
    if 'intents' in lex_bot:
        intent_list = create_lex_intents(lex_bot['intents'], account_id, slot_type_version)
...
def create(event, _):
    ...
    if 'BotName' in event['ResourceProperties']:
